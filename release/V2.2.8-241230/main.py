...
class gc500_modbus(object):
    ...
    # 称重桶状态上报
    def rub_stat_send_server_msg(self, imei, weight, barrelNo, dropTime, personID, wasteType):
        iccid = sim.getIccid()
        data = {
    ...
    # 称重设备上线上报
    def rub_start_send_server_msg(self, imei, dropTime):
        iccid = sim.getIccid()
        data = {
            "method": "thing.event.property.post",
            "id": "0",
    ...
    def uart_parse(self, imei, device_address, recv_data, overflow_limit, overflow_num):
        if len(recv_data) >= 15 and recv_data[0] == int(device_address): #判定数据帧至少15bytes
            if recv_data[1] == 0x43: #接收到控制板状态回复帧
                print('recv 0x43 data:', recv_data)
                # 称重数据
                weight = recv_data[5] + (recv_data[6] << 8)
                # 种类
                if recv_data[7] == 1:
                    wasteType = 3
                elif recv_data[7] == 2:
                    ...
                    wasteType = recv_data[7] 

                barrelNo = 1
                dropTime = utime.mktime(utime.localtime())
                personID = 0

                return self.rub_stat_send_server_msg(self, imei, weight, barrelNo, dropTime, personID, wasteType)
...
class aptu_cls(singleton):
    ...
    @classmethod
    def __init__(cls, projectName="SDTU2100-GC610E", projectVersion="V2.2.7"):
        try:
...
class mqtt_client(aptu_cls):
    ...
    def __up_transaction_thread(argv):
        ...
        recv = gc500_modbus.rub_start_send_server_msg(gc500_modbus, imei, dropTime)
        print("recv : ", recv)
